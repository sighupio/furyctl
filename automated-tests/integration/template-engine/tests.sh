...
@test "no distribution file" {
    info
    test_dir="./automated-tests/integration/template-engine/test-data/no-distribution-yaml"
...
@test "no furyctl.yaml file" {
    info
    test_dir="./automated-tests/integration/template-engine/test-data/no-furyctl-yaml"
    furyctl_bin=${PWD}/dist/furyctl-${OS}_${OS}_${CPUARCH}/furyctl
    init(){
        cd ${test_dir} && ${furyctl_bin} -d --debug template
    }
...
@test "no data property in distribution.yaml file" {
    info
    test_dir="./automated-tests/integration/template-engine/test-data/distribution-yaml-no-data-property"
    furyctl_bin=${PWD}/dist/furyctl-${OS}_${OS}_${CPUARCH}/furyctl
    init(){
        cd ${test_dir} && ${furyctl_bin} -d --debug template
    }
...
@test "empty template" {
    info
    test_dir="./automated-tests/integration/template-engine/test-data/empty"
    furyctl_bin=${PWD}/dist/furyctl-${OS}_${OS}_${CPUARCH}/furyctl
    init(){
        cd ${test_dir} && ${furyctl_bin} -d --debug template
...
@test "complex template dry-run" {
    info
    test_dir="./automated-tests/integration/template-engine/test-data/complex"
    furyctl_bin=${PWD}/dist/furyctl-${OS}_${OS}_${CPUARCH}/furyctl
    init(){
        cd ${test_dir} && ${furyctl_bin} -d --debug template --dry-run
