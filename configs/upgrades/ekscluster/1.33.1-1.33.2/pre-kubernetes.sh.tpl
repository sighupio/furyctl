...
$terraformbin -chdir=kubernetes/terraform state pull > /tmp/kubernetes-state-${timestamp}.json  
  
# Upload to S3 with .bkp extension  
aws s3 cp /tmp/kubernetes-state-${timestamp}.json \  
  s3://${s3bucket}/${s3keyprefix}/kubernetes.${timestamp}.bkp \  
  --region ${s3region}  
