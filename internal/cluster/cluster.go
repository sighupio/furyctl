...
func New(opts *Options) (c *Cluster, err error) {
	...
	if err != nil {
		logrus.Errorf("Error creating the cluster instance while acquiring the right provisioner: %v", err)
		return nil, err
	}
...
func (c *Cluster) Init(reset bool) (err error) {
	...
	err = prov.Prepare()
	if err != nil {
		logrus.Errorf("error while preparing provisioner environment: %v", err)
		return err
	}
...
func (c *Cluster) Update(dryrun bool) (err error) {
	...
	err = c.project.CreateSubDirs(clusterProjectDefaultSubDirs)
	if err != nil {
		logrus.Warnf("error while initializing project subdirectories: %v", err)
	}
	...
	if !dryrun {
		...
		if err != nil {
			logrus.Errorf("Error while updating the cluster. Take a look to the logs. %v", err)
			return err
		}
		c.s.Stop()
		...
		output, err = c.output()
		if err != nil {
			logrus.Errorf("Error while getting the output with the cluster data: %v", err)
			return err
		}
		...
		if err != nil {
			logrus.Errorf("Error while writing the output.json to the project directory: %v", err)
			return err
		}
		c.s.Stop()
		...
		if err != nil {
			logrus.Errorf("Error while writing the kubeconfig to the project directory: %v", err)
			return err
		}
		c.s.Stop()
		...
		err = prov.Plan()
		if err != nil {
			logrus.Errorf("[DRYRUN] Error while updating the cluster. Take a look to the logs. %v", err)
			return err
		}
		c.s.Stop()
		proj := *c.project
...
func (c *Cluster) Destroy() (err error) {
	...
	err = prov.Destroy()
	if err != nil {
		logrus.Errorf("Error while destroying the cluster. Take a look to the logs. %v", err)
		return err
	}
	c.s.Stop()
	c.postDestroy()
