...
type Info struct {
	...
	// SDOngoingUpgradeError is set when the upgrade state could not be read. A nil
	// SDOngoingUpgrade means "no upgrade in progress" only when this is empty: otherwise
	// furyctl simply could not tell, and reporting that as "None" would be a lie.
	SDOngoingUpgradeError   string        `json:"sdOngoingUpgradeError,omitempty" yaml:"sdOngoingUpgradeError,omitempty"`
