/**
 * Copyright (c) 2017-present SIGHUP s.r.l All rights reserved.
 * Use of this source code is governed by a BSD-style
 * license that can be found in the LICENSE file.
 */

// User-visible strings of the UI chrome. Wizard labels come from the wizard file and are
// resolved with text(). Adding a language = adding a dictionary here and a code to LANGS.

const DICTS = {
  en: {
    "app.title": "drydock",
    "app.subtitle": "Configuration wizard for SIGHUP Distribution",
    "app.footer": "furyctl drydock · writes a furyctl.yaml for SIGHUP Distribution",
    "yaml.show": "Show YAML",
    "yaml.hide": "Hide YAML",
    "yaml.close": "Close",
    "tip.example": "Example",
    "tip.use": "Use this example",
    "tip.open": "What goes here?",
    "tip.close": "Close",
    "picker.noVersions": "No release of this provider is covered by a wizard.",
    "steps.done": "done",
    "steps.current": "current",
    "steps.locked": "not yet",
    lang: "Language",
    "picker.lead": "Pick the provider and the distribution version to configure.",
    "picker.kind": "Provider",
    "picker.version": "Distribution version",
    "picker.start": "Start",
    "picker.loading": "Downloading the distribution…",
    "nav.back": "Back",
    "nav.next": "Next",
    "nav.review": "Review",
    "preview.title": "furyctl.yaml",
    "preview.empty": "The file appears here as you answer.",
    "preview.templateError": "The wizard template failed. This is a drydock bug; the message is below.",
    "status.idle": "Ready",
    "status.working": "Rendering…",
    "status.ok": "Valid",
    "status.todo": "{n} left to fill",
    "status.fix": "{n} to fix",
    "status.todoFix": "{n} left to fill · {m} to fix",
    "errors.todo": "Still to fill",
    "errors.fix": "To fix",
    "source.value": "Written in the file",
    "source.env": "From an environment variable",
    "source.file": "The contents of a file",
    "source.path": "A path, resolved when applied",
    "source.http": "Fetched from a URL",
    "source.pick": "Where does this value come from?",
    "source.chip.value": "abc",
    "source.chip.env": "env",
    "source.chip.file": "file",
    "source.chip.path": "path",
    "source.chip.http": "url",
    "source.why.value": "The value itself, in plain sight.",
    "source.why.env": "For a secret: furyctl reads the variable when it applies.",
    "source.why.file": "For a certificate or a manifest: the file's content is put in its place.",
    "source.why.path": "For a directory or a file furyctl only needs to find.",
    "source.why.http": "For something published elsewhere, fetched when applied.",
    "source.shape.value": "value",
    "source.shape.env": "{env://NAME}",
    "source.shape.file": "{file://./path}",
    "source.shape.path": "{path://./path}",
    "source.shape.http": "{https://…}",
    "file.create": "Create the file…",
    "file.title": "Create a file",
    "file.path": "Path",
    "file.content": "Contents",
    "file.hint": "Written next to the configuration, readable only by you. Nothing above that directory can be written.",
    "file.save": "Write it",
    "file.replace": "Replace it",
    "file.cancel": "Cancel",
    "file.exists": "That file is already there. Replace it only if you mean to.",
    "review.created": "Files drydock wrote for you",
    "source.envName": "VARIABLE_NAME",
    "source.filePath": "./path/to/file",
    "source.url": "https://…",
    "keyValue.key": "key",
    "keyValue.value": "value",
    "table.more": "More…",
    "list.add": "Add",
    "list.remove": "Remove",
    "nodes.role.cp": "Control plane",
    "nodes.role.etcd": "etcd",
    "nodes.role.lb": "Load balancers",
    "nodes.role.infra": "Infrastructure workers",
    "nodes.role.workers": "Application workers",
    "nodes.firstIp": "First IP",
    "nodes.fill": "Fill IPs",
    "nodes.override": "This role differs from the defaults",
    "nodes.rowOverride": "Override…",
    "nodes.hostname": "Hostname",
    "nodes.mac": "MAC address",
    "nodes.ip": "IP address",
    "nodes.dhcp": "DHCP",
    "nodes.missingMac": "MAC address of {host}",
    "nodes.missingIp": "IP address of {host}",
    "steps.missing": "{n} to fill",
    "review.missing": "Fill these in before writing",
    "review.goToStep": "Go to {step}",
    "disk.title": "Add a data disk",
    "disk.device": "Disk",
    "disk.deviceHelp": "Prefer a stable path under /dev/disk/by-id: device names like /dev/sdb can move between boots. The disk is wiped.",
    "disk.purpose": "What it is for",
    "disk.purpose.containerd": "Container runtime (/var/lib/containerd)",
    "disk.purpose.kubelet": "Kubelet (/var/lib/kubelet)",
    "disk.purpose.etcd": "etcd, control plane nodes (/var/lib/etcd)",
    "disk.purpose.custom": "Another mount point",
    "disk.mount": "Mount point",
    "disk.format": "Filesystem",
    "disk.add": "Add this disk",
    "disk.result": "Adds one disk with a single partition and the filesystem that mounts it, below.",
    "review.title": "Review",
    "review.intro": "Open the YAML to check the file, then write it.",
    "review.env": "Environment variables to export",
    "review.file": "Files to prepare",
    "review.path": "Paths resolved at apply time",
    "review.http": "URLs fetched at apply time",
    "review.todo": "Before furyctl apply",
    "review.todo.pki": "Run `furyctl create pki` to populate {pkiPath}.",
    "review.todo.mac": "Fill in the MAC address of: {hosts}.",
    "review.todo.validate": "Run `furyctl validate config` once variables and files are in place.",
    "review.todo.dex": "Complete the Dex connector placeholder under distribution.modules.auth.dex.connectors.",
    "review.blocked": "Fix the schema errors before writing.",
    "review.write": "Write furyctl.yaml",
    "review.written": "Written to {path}. furyctl has stopped; you can close this tab.",
    "review.exists": "The output file already exists. Move it away and restart drydock, or use --output.",
    "error.generic": "Something went wrong: {message}",
  },
};

export const LANGS = Object.keys(DICTS);

let lang = "en";
try {
  const stored = localStorage.getItem("drydock.lang");
  if (stored && DICTS[stored]) lang = stored;
} catch {}

export function getLang() {
  return lang;
}

export function setLang(code) {
  if (!DICTS[code]) return;
  lang = code;
  try {
    localStorage.setItem("drydock.lang", code);
  } catch {}
}

export function t(key, vars = {}) {
  const s = DICTS[lang][key] ?? DICTS.en[key] ?? key;
  return s.replace(/\{(\w+)\}/g, (_, k) => (k in vars ? String(vars[k]) : `{${k}}`));
}

// A wizard Text: {en: "...", it: "..."}. Falls back to English, then to the empty string.
export function text(obj) {
  if (!obj) return "";
  if (typeof obj === "string") return obj;
  return obj[lang] ?? obj.en ?? "";
}
