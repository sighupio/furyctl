	...
	_ = Describe("furyctl", func() {
		...
		Context("config validation", func() {
			...
			It("should exit without errors when config validation succeeds", func() {
				...
				err := os.WriteFile(sslCert, []byte("dummy-cert-content"), 0644)
				Expect(err).To(Not(HaveOccurred()))
				
				err = os.WriteFile(sslKey, []byte("dummy-key-content"), 0644)
				Expect(err).To(Not(HaveOccurred()))
				
				err = os.WriteFile(sslCA, []byte("dummy-ca-content"), 0644)
				Expect(err).To(Not(HaveOccurred()))
				...
				err = os.WriteFile(tempConfigPath, []byte(modifiedConfig), 0644)
				Expect(err).To(Not(HaveOccurred()))
				...
				err = os.WriteFile(filepath.Join(tempDir, "kfd.yaml"), []byte(kfdContent), 0644)
				Expect(err).To(Not(HaveOccurred()))
				
				// Copy furyctl-defaults.yaml
				defaultsContent := FileContent(filepath.Join(originalDir, "furyctl-defaults.yaml"))
				err = os.WriteFile(filepath.Join(tempDir, "furyctl-defaults.yaml"), []byte(defaultsContent), 0644)
				Expect(err).To(Not(HaveOccurred()))
		...
		Context("template dump", func() {
			...
			It("fails if no distribution yaml is found", func() {
				...
				Expect(err).To(HaveOccurred())
				Expect(out).To(ContainSubstring("kind is not valid"))
			})
