...
@test "Bootstrap init" {
    info
    init(){
        ./dist/furyctl-${OS}_${OS}_${CPUARCH}/furyctl --no-tty -d --debug bootstrap init --config ./test/integration/aws-eks/bootstrap.yml -w ./test/integration/aws-eks/bootstrap --reset
    }
    run init
...
@test "Bootstrap structure" {
    info
    project_dir="./test/integration/aws-eks/bootstrap"
    test(){
...
@test "Cluster init" {
    info
    init(){
        ./dist/furyctl-${OS}_${OS}_${CPUARCH}/furyctl --no-tty -d --debug cluster init --config ./test/integration/aws-eks/cluster.yml -w ./test/integration/aws-eks/cluster --reset
    }
    run init
    if [[ ${status} -ne 0 ]]; then
        echo "${output}" >&3
...
@test "Cluster structure" {
    info
    project_dir="./test/integration/aws-eks/cluster"
    test(){
